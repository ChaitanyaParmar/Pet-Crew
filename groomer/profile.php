...
<body>
  ...
  if (isset($_POST["btnsub"])) {
    ...
    if ($_FILES['file']['name'] != "") {
      ...
      if (in_array($fileActualExt, $allow)) {
        if ($fileError == 0) {
          if ($fileSize < 500000) {
            ...
            $fileDestination = "images/groomers/" . $fileNameNew;
      ...
      $qry = "update groomer set fullname='$tname',contactno='$contactno',gender='$gender',email='$temail',shopname = '$tsname', address='$address',img='$fileNameNew' where gid='$id'";
      $result = $cnn->query($qry);
      $yourURL = "profile.php";
      echo ("<script>location.href='$yourURL'</script>");
    } else {
      $qry = "update groomer set fullname='$tname',contactno='$contactno',gender='$gender',email='$temail',shopname = '$tsname', address='$address' where gid='$id'";
      $result = $cnn->query($qry);
      if ($result) {
        ...
        <tr>
          <td>
            ...
            if (empty($img)) {
              ...
              echo "<img width='180px' height='160px' style='border-radius:10%; position:relative; left:200px; top:-20px;'src='images/groomers/$img'>";
            }
            ?>
