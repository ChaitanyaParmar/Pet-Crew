...
<head>
  ...
  <link rel="icon" href="user/images/logo/favicon.ico">
</head>
<style media="screen">
  * {
    margin: 0;
...
if (isset($_POST["btnlogin"])) {
  ...
  if (strlen($pass) < 6 || strlen($pass) > 15) {
    $password_error = "Password must be of 6 - 15 characters";
    $error = 1;
  }
  ...
  if (strstr($uname, 'admin')) {
    ...
    if ($count > 0) {
      ...
      $_SESSION["uname"] = $uname;
      if(isset($_SESSION["uname"])){
        header("location: manageadmin/home.php");
      }
    } else {
      $error = "Invalid Username or Password!";
    }
  ...
  else if (strstr($uname, 'tr')) {
    $qry = "Select * from trainer where uname = '$uname' and pwd = '$password'";
    $result = $conn->query($qry);
    $count  = mysqli_num_rows($result);
    if ($count > 0) {
      $row = $result->fetch_assoc();
  ...
  else if (strstr($uname, 'dr')) {
    $qry = "Select * from doctor where uname = '$uname' and pwd = '$password'";
    $result = $conn->query($qry);
    $count  = mysqli_num_rows($result);
    if ($count > 0) {
      $row = $result->fetch_assoc();
  ...
  else if (strstr($uname, 'gr')) {
    $qry = "Select * from groomer where uname = '$uname' and pwd = '$password'";
    $result = $conn->query($qry);
    $count  = mysqli_num_rows($result);
    if ($count > 0) {
      $row = $result->fetch_assoc();
      $_SESSION["id"] = $row["gid"];
      $_SESSION["uname"] = $row["uname"];
      header("Location: groomer/index.php");
    } else {
      $error = "Invalid Username or Password!";
    }
  }

  else if (strstr($uname, 'sh')) {
      $qry = "Select * from shop where uname = '$uname' and pwd = '$password'";
      $result = $conn->query($qry);
      $count  = mysqli_num_rows($result);
      if ($count > 0) {
          $row = $result->fetch_assoc();
          $_SESSION["id"] = $row["shid"];
          $_SESSION["uname"] = $row["uname"];
...
<body>
  <section>
    <div class="imgBx">
      <img src="user/images/resources/bg.jpg" alt="">
    </div>
    <div class="contentBx">
      <div class="formBx">
        ...
        <form method="POST">
          ...
          <div class="inputBx">
            ...
            <p style="margin-top: 10px;">Want to Join as Professional? <a href="projoin.php">Click Here</a></p>
          </div>
        </form>
      </div>
    </div>
